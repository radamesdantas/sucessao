...
import { updateBehaviorModule } from '@/lib/storage';
...
export default function BehaviorModuleDetailPage() {
  ...
  useEffect(() => {
    if (data) {
      const mod = (data.behaviorModules || []).find((m) => m.id === moduleId);
      if (mod) {
  ...
  const mod = (data.behaviorModules || []).find((m) => m.id === moduleId);
  if (!mod) {
    return <div className="text-zinc-500">Comportamento nao encontrado</div>;
  }
  ...
  const handleWeekStatus = (weekNum: 1 | 2 | 3 | 4, status: 'pending' | 'in_progress' | 'completed') => {
    ...
    updateBehaviorModule(menteeId, moduleId, { weeks: updatedWeeks, status: moduleStatus } as never);
    refreshData();
  };
  ...
  const handleSaveNotes = (weekNum: number) => {
    ...
    updateBehaviorModule(menteeId, moduleId, { weeks: updatedWeeks } as never);
    refreshData();
  };
  ...
  return (
    <div className="space-y-6">
      <button
        onClick={() => router.push('/cces')}
        className="flex items-center gap-2 text-sm text-zinc-400 hover:text-white transition-colors"
      >
        <ArrowLeft className="h-4 w-4" />
        Voltar a Comportamentos
      </button>
